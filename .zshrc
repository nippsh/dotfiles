...
git_checkout_issue() {
  number="$1"
  branch="$(git branch -r --list "*$number-*" | sed -e 's/^..//')"
  local_branch_name="$(echo "$branch" | cut -d'/' -f 2-)"

  if [ -z "$branch" ]; then
    echo "No branch found with id $number"
  else
    ...
    if git show-ref --verify --quiet refs/heads/"$local_branch_name"; then
      ...
      # if not, create a new tracking branch
...
alias napi='npm run build-swagger-api'

# Autocompletion for nr command
...
_npm_scripts() {
  ...
  local -a scripts
  scripts=($(npm run 2>/dev/null | awk 'NR>1 && !/lifecycle/ {print $1}'))
  _describe 'npm scripts' scripts
}
compdef _npm_scripts nr
...
## COUNT FILES RECURSIVELY CURRENT DIRECTORY
alias cf="find . -maxdepth 1 -type f | wc -l" # count files in current directory 
alias cfr="find . -type f | wc -l" # include subdirectories (cfr = count files recursively)
